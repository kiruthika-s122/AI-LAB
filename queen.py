   ...
   def conflicts(self,assignment):
      ...
      for i in range(self.N):
          for j in range(i+1,self.N):
              if assignment[i]==assignment[j] or abs(assignment[i]-asignment[j])==j-i:
                  count+=1
      return count
    def min_conflicts(self,max_steps=1000):
        ...
        for_in range(max_steps):
            ...
            min_conflicts_value=min(self.domains,key=lambda val:self.conflicts(assignment[:var]+[val]+assignment[var+1:]))
